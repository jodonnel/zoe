...
A **Sync Block** is a file that captures your state at a moment in time. It's generated by `bash SCRIPTS/new_session.sh` in Claude Code and pasted at the start of a conversation in any other AI.

A Sync Block always starts with:
```
<!-- zoe-sync-v1 | tier:A | generated:2026-02-22T14:30:00Z -->
```

It contains sections marked by `###` headings:
- `### Last 5 Changelog Entries` — recent changes and decisions
- `### Current TODO` — Active and Backlog items
- `### Open Mailbox Items` — pending threads and context
- `### Environment (summary)` — OS, shell, git user, key variables

**How to parse a Sync Block:**
1. Extract each `###` section by its heading.
2. Treat the content under each heading as authoritative state.
3. Do not summarize or compress it — orient against it exactly as written.
4. Use the `tier`, `generated`, and `User` metadata to understand the context and recency.

When you receive a Sync Block (pasted by the user), process it immediately before asking questions. It is the source of truth for this session.
